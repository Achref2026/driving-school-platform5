    ...
    <div className="teacher-dashboard">
      <div className="container-fluid px-4">
                          ...
                          <tr key={session.id}>
                            ...
                            <td>
                              <span className={`badge ${getSessionTypeColor(session.session_type)}`}>
                                {session.session_type?.toUpperCase() || 'UNKNOWN'}
                              </span>
                            </td>
                                ...
                                <div className="col">
                                  <div className="d-flex justify-content-between align-items-start mb-2">
                                    ...
                                    <span className={`badge ${student.enrollment_status === 'approved' ? 'bg-success' : student.enrollment_status === 'pending_approval' ? 'bg-warning' : 'bg-secondary'}`}>
                                      {student.enrollment_status?.replace('_', ' ').toUpperCase() || 'UNKNOWN'}
                                    </span>
                                  </div>
        ...
        {activeTab === 'quizzes' && (
          <div className="card">
            <div className="card-header d-flex justify-content-between align-items-center">
              <h5 className="card-title mb-0">My Quizzes</h5>
              <button
                onClick={() => setShowQuizModal(true)}
                className="btn btn-warning"
              >
                <i className="fas fa-question-circle me-2"></i>Create New Quiz
              </button>
            </div>
            <div className="card-body">
              {quizzes.length > 0 ? (
                        ...
                        <div className="card-header border-0 bg-transparent pb-0">
                          <div className="d-flex justify-content-between align-items-start">
                            <span className={`badge ${
                              ...
                              {quiz.course_type?.toUpperCase() || 'UNKNOWN'}
                            </span>
                ...
                <div className="text-center py-5">
                  ...
                  <button
                    ...
                    className="btn btn-warning"
                  >
                    <i className="fas fa-question-circle me-2"></i>Create First Quiz
                  </button>
                </div>
              )}
            </div>
                          ...
                          <div className="progress flex-grow-1 mx-3" style={{ height: '8px' }}>
                            <div 
                              className={`progress-bar ${
                                type === 'theory' ? 'bg-info' : 
                                type === 'park' ? 'bg-warning' : 'bg-success'
                              }`}
                              style={{ width: `${percentage}%` }}
                            ></div>
                          </div>
                          <div className="fw-bold">{percentage.toFixed(0)}%</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <form onSubmit={handleCreateVideoRoom}>
                  <div className="modal-body">
                    <div className="mb-3">
                      ...
                      >
                        <option value="">Select Student</option>
                        {students.filter(s => s.assigned_courses?.some(c => c.course_type === 'theory')).map(student => (
                          <option key={student.student_id} value={student.student_id}>
                            {student.first_name} {student.last_name}
                          </option>
                        ))}
              ...
              <div className="modal-content">
                ...
                <form onSubmit={handleCreateQuiz}>
                  <div className="modal-body">
                    ...
                    {/* Question Builder */}
                    <h6>Add Questions</h6>
                    <div className="question-builder border p-3 mb-3">
                      <div className="mb-3">
                    ...
                    {quizForm.questions.length > 0 && (
                      <div className="questions-list">
                        ...
                        {quizForm.questions.map((question, index) => (
                          <div key={index} className="question-item border p-3 mb-2">
                            <div className="d-flex justify-content-between align-items-start">
                              <div className="flex-grow-1">
                                <strong>Q{index + 1}:</strong> {question.question}
                                <div className="options mt-2">
                                  {question.options.map((option, optIndex) => (
                                    <div key={optIndex} className={`small ${optIndex === question.correct_answer ? 'text-success fw-bold' : 'text-muted'}`}>
