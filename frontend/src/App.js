...
function App() {
  ...
  // Fetch current user data from backend to check for role changes
  const refreshUserData = async () => {
    try {
      ...
      if (user && user.role !== currentUser.role) {
        console.log(`User role changed from ${user.role} to ${currentUser.role}`);
        // Update user data in localStorage and state
        localStorage.setItem('userData', JSON.stringify(currentUser));
        setUser(currentUser);
        
        // Show success message for role change
        setSuccessMessage(`Congratulations! Your account has been approved. You are now a ${currentUser.role}.`);
        
        // Redirect to appropriate dashboard
        setCurrentPage('dashboard');
        
        // Fetch appropriate data for new role
  ...
  const handleEnroll = async (schoolId) => {
    ...
    try {
      ...
      setCurrentPage('dashboard');
      // Fetch updated data after enrollment
      await fetchDashboardData();
      await fetchUserDocuments();
    } catch (error) {
  ...
  // Periodically check for role changes (every 30 seconds when user is logged in)
  useEffect(() => {
    if (!user) return;
    ...
    // Also check immediately when dashboard loads
  ...
  const renderDashboard = () => {
    ...
    // If user is a manager, render the new enhanced ManagerDashboard with comprehensive functionality
          ...
          <div className="dashboard-content">
            <div className="row g-4">
              <div className="col-lg-8">
                ...
                {/* Document Upload Alert for Users with Enrollments */}
                {user && dashboardData?.enrollments && dashboardData.enrollments.length > 0 && (
                  <div className="card shadow-sm mb-4 border-warning">
                    <div className="card-header bg-warning bg-opacity-10 border-warning">
                      <h3 className="card-title fw-bold mb-0 text-warning">
                    ...
                    <div className="card-body">
                      <div className="row align-items-center">
                        <div className="col-md-8">
                          <h5 className="fw-bold mb-2">Complete your enrollment by uploading documents</h5>
                          <p className="text-muted mb-3">
                            To proceed with your driving lessons, please upload the required documents for verification.
                            Your enrollment will be approved once all documents are reviewed by the manager.
                          </p>
                          <div className="required-docs-list">
                            <small className="text-muted">Required documents:</small>
                            <div className="d-flex flex-wrap gap-2 mt-1">
                              <span className="badge bg-secondary">Profile Photo</span>
                              <span className="badge bg-secondary">ID Card</span>
                              <span className="badge bg-secondary">Medical Certificate</span>
                              <span className="badge bg-secondary">Residence Certificate</span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="quick-actions d-grid gap-3">
                      {user?.role === 'guest' && (
                        <>
                          ...
                          {/* Show upload documents button for guests with enrollments */}
                          {dashboardData?.enrollments && dashboardData.enrollments.length > 0 && (
                            <button
                              onClick={() => setDocumentUploadModal(true)}
