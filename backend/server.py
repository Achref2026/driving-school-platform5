...
class Enrollment(BaseModel):
    ...
    assigned_teacher_id: Optional[str] = None  # New field for teacher assignment
    created_at: datetime
...
async def create_sequential_courses(enrollment_id: str):
    ...
    # Get enrollment to check if teacher is already assigned
    enrollment = await db.enrollments.find_one({"id": enrollment_id})
    assigned_teacher_id = enrollment.get("assigned_teacher_id") if enrollment else None
    ...
    for i, course_config in enumerate(course_configs):
        ...
        course_doc = {
            ...
            "teacher_id": assigned_teacher_id,  # Assign teacher if already assigned to enrollment
            "scheduled_sessions": [],
...
async def get_current_user_info(current_user: dict = Depends(get_current_user)):
    """Get current user information (useful for checking role changes)"""
    try:
        # Return current user data (exclude password hash)
...
async def get_user_documents(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # For guests, show student requirements since they will become students
        user_role = current_user["role"]
        if user_role == "guest":
            required_docs = REQUIRED_DOCUMENTS.get(UserRole.STUDENT, [])
        else:
            required_docs = REQUIRED_DOCUMENTS.get(user_role, [])
...
async def create_enrollment(
    ...
    try:
        ...
        enrollment_doc = {
            ...
            "driving_school_id": enrollment_data.school_id,
            "enrollment_status": EnrollmentStatus.PENDING_APPROVAL,
            "assigned_teacher_id": None,  # Will be assigned by manager
            "created_at": datetime.utcnow(),
            "approved_at": None
        }
        ...
        # NOTE: User remains as 'guest' until manager approves enrollment
        # Role will change to 'student' only after manager approval
...
async def upload_document(
    ...
    try:
        ...
        # Allow both guests and students to upload documents
        user_role = current_user["role"]
        if user_role in ["guest", "student"]:
            # For guests uploading documents, check against student requirements 
            # since they will become students after approval
            check_role = "student" if user_role == "guest" else user_role
    ...
    try:
        ...
        for enrollment in enrollments:
            ...
            if student:
                ...
                enrollment["student_gender"] = student["gender"]
            ...
            # Get assigned teacher information if exists
            if enrollment.get("assigned_teacher_id"):
                teacher = await db.teachers.find_one({"id": enrollment["assigned_teacher_id"]})
                if teacher:
                    ...
                    if teacher_user:
                        ...
                        enrollment["assigned_teacher"] = None
                else:
                    enrollment["assigned_teacher"] = None
            else:
                enrollment["assigned_teacher"] = None
...
async def accept_student_enrollment(
    ...
    try:
        ...
        # IMPORTANT FIX: Update user role from 'guest' to 'student' when enrollment is approved
...
# Teacher Assignment for Enrollments
@api_router.post("/manager/enrollments/{enrollment_id}/assign-teacher")
async def assign_teacher_to_enrollment(
    enrollment_id: str,
    teacher_id: str = Form(...),
    current_user = Depends(get_current_user)
):
    """Assign a teacher to a student enrollment - this teacher will teach all courses"""
    try:
        if current_user["role"] != "manager":
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to assign teacher for this enrollment")
        ...
        if not teacher:
            raise HTTPException(status_code=404, detail="Teacher not found or not approved in this school")
        
        # Get student details to check gender compatibility
        student = await db.users.find_one({"id": enrollment["student_id"]})
        teacher_user = await db.users.find_one({"id": teacher["user_id"]})
        
        if not student or not teacher_user:
            raise HTTPException(status_code=404, detail="Student or teacher user details not found")
        
        # Check gender teaching rules
        if student["gender"] == "female" and not teacher.get("can_teach_female", True):
            raise HTTPException(status_code=400, detail="This teacher cannot teach female students")
        
        # Assign teacher to enrollment
        await db.enrollments.update_one(
            {"id": enrollment_id},
            {
                "$set": {
                    "assigned_teacher_id": teacher_id,
                    "teacher_assigned_at": datetime.utcnow(),
                    "teacher_assigned_by": current_user["id"]
                }
            }
        )
        
        # Update all courses for this enrollment to have the assigned teacher
        ...
        notification_doc = {
            ...
            "message": f"Your teacher {teacher_user['first_name']} {teacher_user['last_name']} has been assigned for all your courses",
            "is_read": False,
            "metadata": {
                "enrollment_id": enrollment_id,
                "teacher_id": teacher_id,
        ...
        teacher_notification_doc = {
            "id": str(uuid.uuid4()),
            "user_id": teacher["user_id"],
            "type": "student_assigned",
            "title": "New Student Assigned",
            "message": f"You have been assigned to teach {student['first_name']} {student['last_name']} all courses",
            "is_read": False,
            "metadata": {
                "enrollment_id": enrollment_id,
                "student_id": enrollment["student_id"],
        ...
        await db.notifications.insert_one(teacher_notification_doc)
...
@api_router.get("/manager/enrollments/{enrollment_id}/available-teachers")
async def get_available_teachers_for_enrollment(
    enrollment_id: str,
    current_user = Depends(get_current_user)
):
    """Get available teachers that can be assigned to this enrollment"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can view available teachers")
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to view teachers for this enrollment")
        
        # Get student details to filter teachers by gender compatibility
        student = await db.users.find_one({"id": enrollment["student_id"]})
        if not student:
            raise HTTPException(status_code=404, detail="Student not found")
        
        # Get all approved teachers from this school
        ...
        # Filter teachers based on gender compatibility and get user details
        available_teachers = []
        for teacher in teachers:
            # Check gender teaching rules
            if student["gender"] == "female" and not teacher.get("can_teach_female", True):
                continue  # Skip teachers who can't teach female students
            ...
            if teacher_user:
                teacher_info = {
                    ...
                    "phone": teacher_user["phone"],
                    "gender": teacher_user["gender"],
                    "can_teach_male": teacher.get("can_teach_male", True),
                    "can_teach_female": teacher.get("can_teach_female", True),
        ...
        logger.error(f"Get available teachers error: {str(e)}")
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail="Failed to get available teachers")
...
@api_router.get("/quizzes/available")
async def get_available_quizzes(current_user: dict = Depends(get_current_user)):
    """Get all available quizzes for students"""
    try:
        if current_user["role"] != "student":
            raise HTTPException(status_code=403, detail="Only students can access available quizzes")
        
        # Get active quizzes
        quizzes_cursor = db.quizzes.find({"is_active": True}).sort("created_at", -1)
        quizzes = await quizzes_cursor.to_list(length=None)
        
        # Get student's quiz attempts to show progress
        attempts_cursor = db.quiz_attempts.find({"student_id": current_user["id"]})
        ...
        for quiz in quizzes:
            quiz_id = quiz["id"]
            if quiz_id in attempts_by_quiz:
                attempt = attempts_by_quiz[quiz_id]
                quiz["attempted"] = True
                quiz["last_score"] = attempt["score"]
                quiz["passed"] = attempt["passed"]
                quiz["attempt_date"] = attempt["completed_at"]
            else:
                quiz["attempted"] = False
                quiz["last_score"] = None
                quiz["passed"] = None
                quiz["attempt_date"] = None
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail="Failed to retrieve available quizzes")
...
async def get_my_quiz_attempts(current_user: dict = Depends(get_current_user)):
    ...
    try:
        if current_user["role"] != "student":
            raise HTTPException(status_code=403, detail="Only students can access their quiz attempts")
        
        # Get student's quiz attempts
        attempts_cursor = db.quiz_attempts.find({"student_id": current_user["id"]}).sort("completed_at", -1)
        attempts = await attempts_cursor.to_list(length=None)
        
        # Get quiz details for each attempt
        quiz_ids = [attempt["quiz_id"] for attempt in attempts]
        quizzes_cursor = db.quizzes.find({"id": {"$in": quiz_ids}})
        quizzes = await quizzes_cursor.to_list(length=None)
        quizzes_by_id = {quiz["id"]: quiz for quiz in quizzes}
        
        # Combine attempt data with quiz info
        detailed_attempts = []
            ...
            if quiz:
                detailed_attempt = {
                    **attempt,
                    ...
                    "quiz_course_type": quiz["course_type"],
                    "quiz_passing_score": quiz["passing_score"]
                }
                detailed_attempts.append(detailed_attempt)
        
        return {
            "attempts": serialize_doc(detailed_attempts),
            "total": len(detailed_attempts)
        }
    
    except Exception as e:
        logger.error(f"Get my quiz attempts error: {str(e)}")
        if isinstance(e, HTTPException):
            raise e
...
async def get_my_quizzes(current_user: dict = Depends(get_current_user)):
    """Get all quizzes created by the current teacher or manager"""
    if current_user["role"] not in ["teacher", "manager"]:
        raise HTTPException(status_code=403, detail="Only teachers and managers can access this endpoint")
...
@api_router.put("/schools/my")
async def update_my_school(
    school_data: DrivingSchoolCreate,
    current_user = Depends(get_current_user)
):
    """Update the manager's own driving school information"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can update school information")
        
        # Find the school owned by this manager
        school = await db.driving_schools.find_one({"manager_id": current_user["id"]})
        if not school:
        ...
        updated_school = await db.driving_schools.find_one({"manager_id": current_user["id"]})
        return serialize_doc(updated_school)
...
async def create_session(
    ...
    try:
        ...
        session_doc = {
            ...
            "course_id": session_data.course_id,
            "teacher_id": session_data.teacher_id,
            "student_id": getattr(session_data, 'student_id', None),
            "session_type": getattr(session_data, 'session_type', "theory"),
...
# TEACHER ASSIGNMENT ENDPOINTS - NEW FUNCTIONALITY

@api_router.get("/manager/available-teachers/{school_id}")
async def get_available_teachers(
    school_id: str,
    current_user = Depends(get_current_user)
):
    """Get available teachers for a specific school"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can access teacher list")
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to access this school's teachers")
        
        # Get approved teachers for this school
        teachers_cursor = db.teachers.find({
        ...
        for teacher in teachers:
            ...
            if user:
                teacher_info = {
                    ...
                    "email": user["email"],
                    "gender": user["gender"],
                    "can_teach_male": teacher["can_teach_male"],
                    "can_teach_female": teacher["can_teach_female"],
                    "rating": teacher.get("rating", 0.0),
                    "total_reviews": teacher.get("total_reviews", 0)
                }
                enriched_teachers.append(teacher_info)
        
        return {"teachers": serialize_doc(enriched_teachers)}
...
@api_router.post("/manager/courses/{course_id}/assign-teacher")
async def assign_teacher_to_course(
    ...
    try:
        ...
        # Get the enrollment to verify school ownership
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to assign teachers for this school")
        
        # Verify teacher exists and is approved for this school
        teacher = await db.teachers.find_one({
            "id": teacher_id,
            "driving_school_id": enrollment["driving_school_id"],
            "is_approved": True
        ...
        if not teacher:
            raise HTTPException(status_code=404, detail="Teacher not found or not approved for this school")
        
        # Get student details for gender matching validation
        student = await db.users.find_one({"id": enrollment["student_id"]})
        if not student:
            raise HTTPException(status_code=404, detail="Student not found")
        ...
        # Get teacher user details for response
        teacher_user = await db.users.find_one({"id": teacher["user_id"]})
        
        # Send notification to student about teacher assignment
        notification_doc = {
            "id": str(uuid.uuid4()),
            "user_id": enrollment["student_id"],
            "type": "teacher_assigned",
            "title": "Teacher Assigned to Your Course",
            "message": f"Teacher {teacher_user['first_name']} {teacher_user['last_name']} has been assigned to your {course['course_type']} course.",
            "is_read": False,
            "metadata": {
                "course_id": course_id,
                "teacher_id": teacher_id,
        ...
        # Send notification to teacher about assignment
        teacher_notification_doc = {
            "id": str(uuid.uuid4()),
            "user_id": teacher["user_id"],
            "type": "student_assigned",
            "title": "New Student Assigned",
            "message": f"You have been assigned to teach {student['first_name']} {student['last_name']} for {course['course_type']} course.",
            "is_read": False,
            "metadata": {
                "course_id": course_id,
                "student_id": enrollment["student_id"],
...
@api_router.delete("/manager/courses/{course_id}/unassign-teacher")
async def unassign_teacher_from_course(
    ...
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can unassign teachers")
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to manage teachers for this school")
        
        # Check if course has a teacher assigned
        if not course.get("teacher_id"):
            raise HTTPException(status_code=400, detail="No teacher assigned to this course")
        ...
        logger.error(f"Unassign teacher error: {str(e)}")
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail="Failed to unassign teacher")

# TEACHER STUDENT VISIBILITY ENDPOINTS - PHASE 1 FIX

@api_router.get("/teacher/my-students")
async def get_teacher_assigned_students(current_user = Depends(get_current_user)):
    """Get all students assigned to the current teacher across all schools"""
    try:
        if current_user["role"] != "teacher":
        ...
        enrollment_ids = list(set([course["enrollment_id"] for course in courses]))
        
        # Get enrollments and student details
        students_data = []
        for enrollment_id in enrollment_ids:
            ...
            student_user = await db.users.find_one({"id": enrollment["student_id"]})
            if not student_user:
                continue
            ...
            # Get student's courses assigned to this teacher
            student_courses = [course for course in courses if course["enrollment_id"] == enrollment_id]
            
            # Get sessions for this student-teacher pair
            ...
            student_info = {
                ...
                "assigned_courses": serialize_doc(student_courses),
                "sessions": serialize_doc(sessions),
        ...
        return {
            "students": serialize_doc(students_data),
            "total_students": len(students_data),
            "total_schools": len(set([s["school_id"] for s in students_data]))
        }
    
    except Exception as e:
        logger.error(f"Get teacher students error: {str(e)}")
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail="Failed to retrieve assigned students")

@api_router.get("/teacher/students")
async def get_teacher_students_legacy(current_user = Depends(get_current_user)):
    """Legacy endpoint for teacher students - redirects to my-students"""
    return await get_teacher_assigned_students(current_user)

# ENHANCED SCHEDULE MANAGEMENT ENDPOINTS - PHASE 2 FIX

@api_router.post("/manager/create-schedule")
async def create_schedule_for_assigned_students(
    teacher_id: str = Form(...),
    student_id: str = Form(...),
    course_id: str = Form(...),
    ...
    """Create a schedule between assigned teacher and student"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can create schedules")
        
        # Get course to verify it exists and belongs to manager's school
        course = await db.courses.find_one({"id": course_id})
        if not course:
            raise HTTPException(status_code=404, detail="Course not found")
        
        # Get enrollment to verify school ownership
        enrollment = await db.enrollments.find_one({"id": course["enrollment_id"]})
        if not enrollment:
            raise HTTPException(status_code=404, detail="Enrollment not found")
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to create schedules for this school")
        
        # Verify teacher is assigned to this course
        if course["teacher_id"] != teacher_id:
            raise HTTPException(status_code=400, detail="Teacher is not assigned to this course")
        
        # Verify student belongs to this enrollment
        if enrollment["student_id"] != student_id:
            raise HTTPException(status_code=400, detail="Student does not belong to this course")
        
        # Get teacher and student details for notifications
        teacher = await db.teachers.find_one({"id": teacher_id})
        teacher_user = await db.users.find_one({"id": teacher["user_id"]}) if teacher else None
        student_user = await db.users.find_one({"id": student_id})
        ...
        student_notification = {
            ...
            "message": f"A {session_type} session has been scheduled with teacher {teacher_user['first_name']} {teacher_user['last_name']} on {datetime.fromisoformat(scheduled_at.replace('Z', '+00:00')).strftime('%B %d, %Y at %H:%M')}.",
            "is_read": False,
            "metadata": {
        ...
        teacher_notification = {
            ...
            "type": "session_scheduled",
            "title": "New Session Scheduled",
            "message": f"A {session_type} session has been scheduled with student {student_user['first_name']} {student_user['last_name']} on {datetime.fromisoformat(scheduled_at.replace('Z', '+00:00')).strftime('%B %d, %Y at %H:%M')}.",
            "is_read": False,
            "metadata": {
                "session_id": session_id,
                "student_id": student_id,
...
@api_router.get("/manager/assigned-teacher-student-pairs/{school_id}")
async def get_assigned_teacher_student_pairs(
    school_id: str,
    current_user = Depends(get_current_user)
):
    """Get all teacher-student pairs that can be scheduled"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can access this endpoint")
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to access this school")
        
        # Get all courses with assigned teachers for this school
        enrollments_cursor = db.enrollments.find({"driving_school_id": school_id})
        enrollments = await enrollments_cursor.to_list(length=None)
        
        pairs = []
        for enrollment in enrollments:
            # Get courses for this enrollment that have assigned teachers
            ...
            for course in courses:
                ...
                pair_info = {
                    ...
                    "teacher_id": teacher["id"],
                    "teacher_name": f"{teacher_user['first_name']} {teacher_user['last_name']}",
                    "teacher_gender": teacher_user["gender"],
                    "student_id": student["id"],
                    "student_name": f"{student['first_name']} {student['last_name']}",
                    "student_gender": student["gender"],
                    "enrollment_id": enrollment["id"],
        ...
        return {
            "pairs": serialize_doc(pairs),
            "total_pairs": len(pairs),
            "school_name": school["name"]
        }
    
    except Exception as e:
        logger.error(f"Get teacher-student pairs error: {str(e)}")
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail="Failed to retrieve teacher-student pairs")
